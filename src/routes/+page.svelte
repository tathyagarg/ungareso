...
<script lang="ts">
  ...
  let query = $state(data.query || "A/RES/80/");
  ...
  <main class="mx-auto max-w-5xl px-6 py-10">
    ...
    {:else}
      {#if data.query === undefined}
        <span class="mb-6 block font-mono text-sm text-stone-500">
          Showing a few recent resolutions. Use the search bar to find a
          specific resolution.
        </span>
      {/if}
      ...
      <div class="flex gap-4">
        {#each Array(colCount) as _}
          <div class="flex flex-1 flex-col gap-4">
            {#each Array(3) as __}
              <div
                class="h-40 animate-pulse border border-black/25 bg-paper"
              ></div>
            {/each}
          </div>
        {/each}
